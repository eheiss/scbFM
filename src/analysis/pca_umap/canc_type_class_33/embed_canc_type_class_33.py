#!/usr/bin/env python3
"""
Embed TCGA 33-type cancer classification samples with all pretrained/preadapted
backbones, then reduce to PCA-50 and UMAP-2D. Saves a compressed .npz with
coordinates + labels for the plotting script.

Intended to run as a SLURM GPU job via embed_canc_type_class_33_job.sh.
"""
...
OUTPUT_DIR  = f"{ROOT}/output/canc_type_class_33/pca_umap"
OUTPUT_PATH = f"{OUTPUT_DIR}/canc_type_class_33_embeddings.npz"
...
TCGA_COHORTS = {f"TCGA-{c}" for c in COHORTS}
...
adata = adata[adata.obs["project_id"].astype(str).isin(TCGA_COHORTS)].copy()
