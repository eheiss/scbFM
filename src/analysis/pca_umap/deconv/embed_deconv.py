#!/usr/bin/env python3
"""
Embed pseudo-bulk deconvolution samples with all pretrained/preadapted
backbones, then reduce to PCA-50 and UMAP-2D. Saves a compressed .npz with
coordinates, cell-type proportions, and tissue labels for the plotting script.

Intended to run as a SLURM GPU job via embed_deconv_job.sh.
"""
...
DATA_PATH   = f"{ROOT}/datasets/pseudo_bulk/pseudo_bulk_RAW.h5ad"
OUTPUT_DIR  = f"{ROOT}/output/deconv/pca_umap"
OUTPUT_PATH = f"{OUTPUT_DIR}/deconv_embeddings.npz"
...
# Support both prop__* obs columns and uns['cell_type_proportion_columns'] mapping
...
if prop_cols:
    ...
    if not mapping:
        raise ValueError("No prop__* obs columns and no uns['cell_type_proportion_columns'] found.")
    cell_types = sorted(mapping.keys())
    prop_cols  = [mapping[ct] for ct in cell_types]

proportions = adata.obs[prop_cols].to_numpy(dtype=np.float32)
...
tissue_labels = adata.obs["tissue_general"].astype(str).values
...
print(f"Tissue breakdown:\n{pd.Series(tissue_labels).value_counts().to_string()}")
